...
public interface MaintenanceRepo extends JpaRepository<Maintenance,String> {
    ...
    List<lk.ijse.spring.entity.Maintenance> getAllUnderMaintenances();

    @Query(value = "SELECT * FROM Maintenance WHERE date BETWEEN :fromDate AND :toDate",nativeQuery = true)
    List<lk.ijse.spring.entity.Maintenance> getAllMaintenancesByDateRange(@Param("fromDate") String fromDate, @Param("toDate") String toDate);
